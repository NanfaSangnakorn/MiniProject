        ...
        if (data.list && data.list.length) {
            return data.list.map((item, index) => {
                return ( 
                    ...
                    < div >  <b> Scientific name: </b> {item.scien} </div >
                        <div > <b > Plant Kingdom: </b> {item.kingdom} </div >
                          <div >  <b> Plant Family: </b > { item.kingdom } </div >
                              <div >  < b > Style: </b> {item.style} </div >

                    <div> < button className = { styles.btn } onClick = {() => buyStudent(item.id) } >
                    Buy { " " } 
                    </button> 
                    </div> 
                    </div>
