...
router
  ...
  .post((req, res) => {
    ...
    newPet.mystyle = req.body.mystyle;
    newPet.zone = req.body.zone;
...
router
  ...
  .put((req, res) => {
    ...
    pets.list[id].mystyle = req.body.mystyle;
    pets.list[id].zone = req.body.zone;
    res.json(pets.list);
  })
  .delete((req, res) => {
...
router.route("/purchase/:petId").post((req, res) => {
  let id = pets.list.findIndex((item) => +item.id == +req.params.petId);
  if (id == -1) {
    res.json({ message: "Pets not found" });
  } else {
    pets.list = pets.list.filter((item) => +item.id !== +req.params.petId);
    res.json(pets.list);
  }
